...
void symbol_resymbolize(uint32_t pbn, int chip, int way, int chnl, int blk, int valid_start, int num_flush, int sb) {
	...
	// Set new symbol and Set dead bit for LBA and PPA (TODO: BF's new start idx is 1)
        ...
        for(int i=0; i<num_valid; i++) {
            for(int b=1; b<SBLK_SZ; b++) {
                for(int p=0; p<PAGE_PER_BLOCK; p++) {
                    ...
                    if(valid_list[i].oob == storage.chip_arr[*way][*chnl].data_blk[*sb][b].page_arr[p].oob) {
                        global_symb.ppa_flag[superblk][gidx] = 0;
                        // TODO: what if more rebloomed pages?
                        // TODO: not invalid but not copy that data?
                    }
                }
            }
        }
...
//        printf("During RB, GC triggered\n");
        ...
        //bloom_gc(pbn, chip, way, chnl, blk, ppn, rebloom_list, num_rebloom, sb);
...
//printf("WRITE LBA %u\n", lba);
	...
	//global_symb.lba_flag[superblk][lba % PAGE_PER_SBLK] = 1;
...
void bloom_read(uint32_t lba) {
	...
	for(int b=blk; b>=0; b--) {
		for(; idx>=0; idx--) {
            ...
            if(global_symb.ppa_flag[superblk][pa] == 1) {
                if(symbol_check(bf_man->bits_per_pg[new_bf_idx], new_bf_idx, hashkey + new_bf_idx, \
